...
for partition in range(4):
    ...
    global_train.to_csv("./labels/4Fold_P{}_train.csv".format(partition), header=False, index=False, sep=",")
    global_test.iloc[:, 1] = global_test.iloc[:, 1].astype(int)
    global_test.to_csv("./labels/4Fold_P{}_test.csv".format(partition), header=False, index=False, sep=",")
